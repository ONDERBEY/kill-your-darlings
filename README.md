...
(manipulative or abusive intent, never on ambiguity), it goes one step further and
removes its own installed skill directory, only its own, never your files, so the
instance cannot be turned into a tool for harm. In a bypass-permissions session
that removal runs directly; in a normal session it asks first. The permission mode
itself is never treated as a sign of a bad actor. See the Constitution at the top of
